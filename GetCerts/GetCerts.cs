...
public class CertSelect
{
    public static void Main(string[] args)
    {
        try 
        {
            ...
            foreach (X509Certificate2 x509 in fcollection)
            {
                try
                {
                    ...
                    string CertName = x509.GetNameInfo(X509NameType.SimpleName,true);
                    bool backslash = CertName.Contains("\\");
                    string exportName = backslash ? CertName.Replace("\\", "-") + ".pkcs12" : CertName + ".pkcs12";
        ...
        {
            Console.WriteLine("Failed to extract certificates: {0}", ex);
        }
    }
}
